...
try:
    ...
    if not subset.empty:
        ...
        print(f"🤖 A IA indicou principalmente: ZONA {ia_zone}")
        print(f"   (Confiança média: {confianca:.2%})")

        if int(ia_zone) == int(real_zone):
            print("\n🏆 SUCESSO! A IA acertou a zona exata do vazamento real.")
        else:
            if 'zone_centroids' in globals():
                ...
                if dist < 400:
                    print("✅ Resultado Positivo: A IA indicou uma zona vizinha/próxima.")
                else:
                    print("❌ A IA errou a localização por uma margem grande.")
    else:
...
try:
    ...
    print("="*40)
    print(f"1. Correlação Temporal (Tendência): {corr_mag:.4f}")
    print(f"   (Capacidade de dizer QUANDO vaza)")
    print("-" * 40)
    print(f"2. Acurácia de Zona (Exact Match):  {acc_real:.2%}")
    print(f"   (Capacidade de dizer ONDE vaza - Zona Exata)")
    print("-" * 40)
    print(f"3. Erro Médio de Distância:         {mean_dist_real:.2f} metros")
