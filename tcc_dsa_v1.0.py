# -*- coding: utf-8 -*-
"""TCC_DSA_v10_rev2.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1jTRO7cLF64iR5H3GWHw6WMcIOxRw2EOl
...
# --- PLOT DA REDE COM DESTAQUE PARA OS SENSORES DE PRESSÃO SCADA ---
# Os sensores são identificados diretamente pelas colunas do df_pressure (já carregado acima)

sensores_pressao = list(df_pressure.columns)

print(f"Total de colunas em df_pressure: {len(sensores_pressao)}")

# Filtra apenas os sensores que correspondem a nós existentes na rede (wn)
# (o dataset costuma incluir também um sensor no reservatório, ex: 'T1', que não é um node de junção)
nos_existentes = set(wn.node_name_list)
sensores_na_rede = [s for s in sensores_pressao if s in nos_existentes]
sensores_fora_da_rede = [s for s in sensores_pressao if s not in nos_existentes]

print(f"Sensores encontrados como nós da rede: {len(sensores_na_rede)}")
if sensores_fora_da_rede:
    print(f"Colunas não correspondentes a nós da rede (ex: reservatório/tanque): {sensores_fora_da_rede}")
...
wntr.graphics.plot_network(
    wn,
    node_attribute=sensores_na_rede,   # lista -> nós destacados recebem cor diferente dos demais
    node_size=50,
    node_range=[0, 1],
    link_width=1,
    title='Topologia da Rede L-Town — Sensores de Pressão SCADA',
    ax=ax
)
...
history_hybrid = model_hybrid.fit(
    ...
    validation_split=0.2, # O Keras separa 20% do X_train para validar
    epochs=100,
    batch_size=256 if gpu_info else 64,
    verbose=1
)
